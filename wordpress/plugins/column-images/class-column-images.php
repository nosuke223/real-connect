...
class Easy_Featured_images {
    ...
    public function load_textdomain() {
    	load_plugin_textdomain( 'column-images', false, dirname( plugin_basename( __FILE__ ) ) . '/lang' );
    }
    ...
    public function enqueue_assets( $page ) {
    	...
    	wp_localize_script( 'efi_scripts', 'efi_strings', array(
    		'browse_images' => __( 'Browse Or Upload An Image', 'column-images' ),
    		'select_image' =>  __( 'Set featured image', 'column-images' ),
    		'ajaxurl' =>  admin_url( 'admin-ajax.php' ),
    		'add_image' =>  __( 'add image', 'column-images' ),
    		'remove' =>  __( 'remove', 'column-images' )
    	));
    ...
    public function table_head( $columns ) {
    	...
    	$new['_post_thumbnail'] = __( 'Image', 'column-images' );
    ...
    public function column_content( $column_slug, $post_id ) {
    	...
    	if ( '_post_thumbnail' == $column_slug ) {
    		...
    		if( has_post_thumbnail( $post_id ) ) {
    			...
    			echo "<a class='efi-choose-image' data-nonce='" . $nonce . "' href='" . get_edit_post_link( $post_id ) . "'> <i class='dashicons dashicons-plus'></i> <br> ". __('add image','column-images') . "</a>";
    		}

    		echo '</div>';

    		echo "<a href='" . get_edit_post_link( $post_id ) . "' data-nonce='" . $nonce . "' class='efi-remove-image'><i class='dashicons dashicons-no'></i> ". __( 'remove','column-images') . "</a>";
