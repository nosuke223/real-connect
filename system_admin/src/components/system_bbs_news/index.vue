<template lang="pug">
  v-col(cols="12")
    ItemTable(:items="items" :headers="headers" btnTitle="新規作成" tableTitle="お知らせ(news)一覧" @rowClicked="gotoShow" @btnClicked="gotoCreate")
</template>
...
export default {
  ...
  methods: {
    gotoShow(data) {
      this.$router.push(`/system_bbs_news/${data.id}`);
    },
    gotoCreate() {
      this.$router.push("/system_bbs_news/create");
    },
  },
};
</script>
